...
def updateOccupancyGrid(occupancy, robotXScaled, robotYScaled, sonarAbsAngle, sonarRelDistance, sonarMax, indexY):
    occupancy = occupancy[indexY]

    # the tolerance for the sonar
    sonarInaccuracyDist = 1
    sonarInaccuracyAng = 0.02

    # for each pixel of the occupancy grid:
    for indexX in range(0, len(occupancy)):
        ...
        if(pixelRobotAngle < sonarAbsAngle + sonarInaccuracyAng and pixelRobotAngle > sonarAbsAngle - sonarInaccuracyAng):
            ...
            if(distanceFromRobot < (sonarRelDistance - sonarInaccuracyDist)):
                ...
                occupancy[indexX] = (occupancy[indexX] + 1) / 2
            ...
            # if the pixel is within tolerance of the sonar:
                ...
                if(sonarRelDistance < sonarMax):
                    ...
                    occupancy[indexX] = (occupancy[indexX] + 0) / 2
    return occupancy
...
if __name__ == '__main__':
    ...
    def animatePlot(i):
        ...
        for sonarNum in range(0, 15):
            ...
            occupancy, robotXScaled, robotYScaled,

            occupancy = pool.map(partial(updateOccupancyGrid, occupancy, robotXScaled, robotYScaled, sonarAbsAngle, sonarRelDistance, sonarMax), range(0, len(occupancy)))
